...
if __name__ == "__main__":
    ...
    sqlite = sqlite3.connect(
        os.path.join(Tyf.__path__[0], "tags.sqlite")
    )

    # sqlite.row_factory = sqlite3.Row
    sqlite.execute(
        "CREATE TABLE IF NOT EXISTS tags(family TEXT, tag INTEGER, key TEXT, "
        "type BLOB, _default BLOB, comment TEXT);"
    )
    sqlite.execute(
    ...
    for family in ["bTT", "xTT", "pTT", "exfT", "gpsT", "itrT"]:
        dic = getattr(sys.modules[__name__], family)
        for key, value in dic.items():
            print((family, key,) + value)
            sqlite.execute(
                "INSERT OR REPLACE INTO "
                "tags(family, tag, key, type, _default, comment) "
